...
public:
    ...
    OsSemaphore(int initialCount, std::string name)
        : count(initialCount), name(std::move(name)) {}
...
private:
    void emit(const std::string &demo, const std::string &actor, const std::string &action,
              const std::string &status, int tick, int value) {
        if (!eventSink) {
            return;
        }
